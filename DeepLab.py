...
tf.flags.DEFINE_string("logs_dir", "logs_deep1030/", "path to logs directory")
...
def train(loss_val, var_list):
    ...
    #return optimizer.apply_gradients(grads)           # return loss_val

    opt = tf.train.GradientDescentOptimizer(FLAGS.learning_rate)
    grads = opt.compute_gradients(loss_val, var_list=var_list)  # 计算梯度,返回权重更新
    clipped_gradients, _ = tf.clip_by_global_norm(grads, max_gradient_norm)
    return opt.apply_gradients(zip(clipped_gradients, var_list))
...
def main(argv=None):
    ...
    loss = tf.reduce_mean(
        tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, #tf.nn.sparse_softmax_cross_entropy_with_logits自动对logits(resize_F8)做了softmax处理.
    ...
    if FLAGS.mode == "train":
        ...
        tiff.imsave('/home/lenovo/2Tdisk/Wkyao/_/2017/crf_build7_1030.tif', Crf)
        ...
        tiff.imsave('/home/lenovo/2Tdisk/Wkyao/_/2017/build7_1030.tif', res1)
...
def open_and_close(img):
    ...
    tiff.imsave('/home/lenovo/2Tdisk/Wkyao/_/2017/open_and_close_1024.tif', img)
