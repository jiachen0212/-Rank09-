...
import BatchNpReader as dataset     # 读.npy!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
tf.flags.DEFINE_string("logs_dir", "logs_deep1101_512/", "path to logs directory")
tf.flags.DEFINE_string("data_dir", "enhance10.31/", "path to dataset")
tf.flags.DEFINE_float("learning_rate", "1e-4", "Learning rate for Adam Optimizer")
tf.flags.DEFINE_string("model_dir", "Model_zoo/", "Path to vgg model mat")
tf.flags.DEFINE_bool('debug', "False", "Debug mode: True/ False")
tf.flags.DEFINE_string('mode', "test", "Mode train/ test/ visualize")  # visualize

FILE_sub = '/home/lenovo/2Tdisk/Wkyao/_/res.npy'
sub = np.load(FILE_sub)
print(sub.shape)
...
def main(argv=None):
    ...
    train_records, valid_records = readNpData.read_dataset(FLAGS.data_dir)  # 数据的转换输入.
    ...
    if FLAGS.mode == "train":
        ...
        for itr in xrange(20000, 25000):  # 修改itr数值,接着之前的模型数量后继续训练.
        ...
        im_2017_list.append(
            np.array([np.array([sub[0:3000, 15000:15106] for i in range(3)])]).transpose(0, 2, 3, 1))
        '''
        for i in range(50):
        ...
        tiff.imsave('/home/lenovo/2Tdisk/Wkyao/_/2017/sub_1101_2w5.tif', res1)
