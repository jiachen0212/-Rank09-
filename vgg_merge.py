...
tf.flags.DEFINE_string("logs_dir", "log1109_3/", "path to logs directory")
tf.flags.DEFINE_string("data_dir", "enhance11.8_3/", "path to dataset")
...
new_2017_1 = new_2017[:, :, 2]
new_2017_2 = new_2017[:, :, 3]
new_2017 = (new_2017_1 + new_2017_2) / 2
...
def main(argv=None):
    ...
    print('annotation', annotation.shape, type(annotation))
    ...
    if FLAGS.mode == "train":
        ...
        softmax_merge = np.load('/home/lenovo/2Tdisk/Wkyao/_/2017/avge/1110vgg.npy')
        im_2017_list = []
        im_2017_list2 = []
        merge_list = []
        b = []
        ...
        im_2017_list.append(
            np.array([np.array([new_2017[0:4000, 15000:15106] for i in range(3)])]).transpose(0, 2, 3, 1))
        ...
        tiff.imsave('/home/lenovo/2Tdisk/Wkyao/_/2017/vgg/crf_vgg_merger_1110.tif', Crf)
        #np.save('/home/lenovo/2Tdisk/Wkyao/_/2017/vgg/softmax_vgg_1109_3.npy', softmax)  # 保存拼接的Softmax为np
        #res1 = np.concatenate(tuple(allImg), axis=1).astype(np.uint8)
        #tiff.imsave('/home/lenovo/2Tdisk/Wkyao/_/2017/vgg/vgg_1109_3.tif', res1)
...
def open_and_close(img):
    ...
    tiff.imsave('/home/lenovo/2Tdisk/Wkyao/_/2017/vgg/open_vgg_merge_1110_1.tif', img)
