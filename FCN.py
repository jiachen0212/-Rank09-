...
# FILE_result = '/home/lenovo/2Tdisk/Wkyao/_/result_1011.tif'
# im_result = tiff.imread(FILE_result)

# FILE_biaozhu = '/home/lenovo/2Tdisk/Wkyao/_/biaozhu_1012.tif'
# im_2017_biaozhu = tiff.imread(FILE_biaozhu)
...
def main(argv=None):
    ...
    train_records, valid_records = scene_parsing.read_dataset(FLAGS.data_dir)  # 参数是一个路径
    ...
    if FLAGS.mode == "train":
        ...
        for i in range(30):
            ...
            # print (im_2017.shape)
        im_2017_list.append(
            np.array([np.array([im_2017[0:5106, 15000:15106, 3] for i in range(3)])]).transpose(0, 2, 3, 1))    # im_2015
        ...
        tiff.imsave('/home/lenovo/2Tdisk/Wkyao/_/2017/2017_result_1012.tif', result)

def resultImgPro():
    after_median = cv2.medianBlur(im_result, 5)
    ...
    for i in range(after_del.shape[0]):
        for j in range(after_del.shape[1]):
            if after_del[i][j] > 0:
                after_del[i][j] = 1
            # print(after_del[i][j])
    tiff.imsave('/home/lenovo/2Tdisk/Wkyao/_/result_afterPro.tif', after_del)
