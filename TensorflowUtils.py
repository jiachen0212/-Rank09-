...
def atrous_conv2d_basic(x, W, bias, rate):
...
def batch_norm_layer(x, train_phase, scope_bn):
    with tf.variable_scope(scope_bn):
        beta = tf.Variable(tf.constant(0.0, shape=[x.shape[-1]]), name='beta', trainable=True)      # offset
        gamma = tf.Variable(tf.constant(1.0, shape=[x.shape[-1]]), name='gamma', trainable=True)    # scale
        axises = np.arange(len(x.shape) - 1)    # 前三层求均值和方差
        batch_mean, batch_var = tf.nn.moments(x, axises, name='moments')    # num*size[0]*size[1],对输入的所有像素做均值和方差
        ema = tf.train.ExponentialMovingAverage(decay=0.99)  # decay close to 1

        def mean_var_with_update():
            ema_apply_op = ema.apply([batch_mean, batch_var])   # 对这个列表进行滑动平均
            with tf.control_dependencies([ema_apply_op]):
                return tf.identity(batch_mean), tf.identity(batch_var)
        ...
        if train_phase == 'train':
            ...
            mean, var = tf.cond(tf.less(1, 2), mean_var_with_update,
                                lambda: (ema.average(batch_mean), ema.average(batch_var)))      # get lambda
