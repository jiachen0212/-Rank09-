...
def tower_loss(loss, scope):    #在这里把loss放到命名空间下.
    print ('the loss is', loss)
    tf.add_to_collection('losses', loss)
    losses = tf.get_collection('losses', scope=scope)    #tf.get_collection获取集合下的loss  losses=loss
    total_loss = tf.add_n(tf.get_collection('losses'))   # P89
...
def train(loss_val, var_list):
    ...
    return optimizer.apply_gradients(grads)  # 更新所有变量
